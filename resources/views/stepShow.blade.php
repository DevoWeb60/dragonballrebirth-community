            ...
            @foreach ($steps as $step)
                <div class="step">
                    ...
                    <div class="thumbnail-container">
                        ...
                        @else
                            <img class="thumbnail"
                                src="https://www.dragonballrebirth.fr/img/aventure/etapes/{{ $storyNumber }}/story-{{ $storyNumber }}-{{ $step->step_number }}.jpg"
                                alt="{{ $story->story_name }}">
                        @endif
                        <div class="location">
                            <p>{{ $step->planet->planet }} <img src="{{ $step->planet->icon }}"
                                    alt="{{ $step->planet->planet }}"></p>
                    ...
                    <div class="conditions">
                        @foreach ($step->conditions as $condition)
                            ...
                            @if ($condition->any_trains_quantity && !$condition->special_trains)
                                <p class="condition">Faire <span
                                        class="number">{{ $condition->any_trains_quantity }}</span>
                                    entrainements&nbsp;<i class="fa-solid fa-dumbbell"></i></p>
                                <hr>
                            @endif
                            @if ($condition->special_trains && !$condition->any_trains_quantity)
                                <p class="condition">
                                    Faire l'entrainement "{{ $condition->train->name }}"
                                    @if (!empty($condition->train->icon))
                                        <img src="{{ $condition->train->icon }}"
                                            alt="{{ $condition->train->name }}" />
                                    @endif
                                </p>
                                <hr>
                            @endif
                            @if ($condition->special_trains && $condition->any_trains_quantity)
                                <p class="condition">
                                    Faire <span class="number">{{ $condition->any_trains_quantity }}</span> fois
                                    l'entrainement "{{ $condition->train->name }}"
                                    @if (!empty($condition->train->icon))
                            ...
                            @if ($condition->object_harvest)
                                <p class="condition">Récupérer <span
                                        class="number">{{ number_format($condition->object_harvest_quantity, 0, ' ', ' ') }}</span>
                                    {{ $condition->object->name }}
                                    &nbsp;<img src="{{ $condition->object->icon }}"
                                        alt="{{ $condition->object->name }}" />
                                </p>
                                <hr>
                            @endif
                            @if ($condition->spell_condition_number)
                                <p class="condition">Utiliser l'attaque de Magie
                                    {{ $condition->spell_condition_number }}</p>
                                <hr>
                            @endif
                    ...
                    @if ($step->enemy)
                        <div class="fight">
                            <h2 class="player">
                                <span>
                                    {{ $step->character->name }}
                                    <img src="{{ $step->character->avatar }}transformations/base/head.png"
                            ...
                            <h2 class="enemy">
                                ...
                                <span class="info">Vitalité :
                                    {{ number_format($step->enemy_health, 0, ' ', ' ') }}</span>
                            </h2>
                        </div>
                    @else
                    ...
                    <div class="rewards">
                        ...
                        @foreach ($step->rewards as $reward)
                            @if ($reward->zenis)
                                <p class="reward-zenis">{{ number_format($reward->zenis, 0, ' ', ' ') }} <span
                                        class="zenis">@include('svg.zenis')</span></p>
                            @endif
                            @if ($reward->character_id)
                                <p class="reward-character">{{ $reward->character->name }}
                                    <img src="{{ $reward->character->avatar }}transformations/base/head.png"
                                        alt="{{ $reward->character->name }}">
                                </p>
                            @endif
                            @if ($reward->object_id)
                                <p class="reward-object">{{ $reward->object->name }}
                                    &nbsp;<img src="{{ $reward->object->icon }}"
                            ...
                            @if ($reward->caps_id)
                                <p class="reward-caps">{{ $reward->caps->name }}
                                    <img src="{{ $reward->caps->scarecities->icon }}"
                                        alt="{{ $reward->caps->name }}" />
                                    {{ $reward->quantity_caps > 1 ? 'x' . $reward->quantity_caps : null }}
                                </p>
                            @endif
                            @if ($reward->experience)
                                <p class="reward-character">
                                    {{ $reward->experience >= 100 ? number_format($reward->experience, 0, ' ', ' ') : $reward->experience . '%' }}
                                    XP pour {{ $reward->rewardsOnCharacter->name }}
                                    <img src="{{ $reward->rewardsOnCharacter->avatar }}transformations/base/head.png"
                                        alt="{{ $reward->rewardsOnCharacter->name }}">
                                </p>
                            @endif
                            @if ($reward->skin)
                                <p class="reward-character">
